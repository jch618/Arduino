...
//밀리스는 아두이노 내부적으로 시간 재는 함수이다.
// 만약 샘플링한 시간에서 인터벌 시간보다 덜 지났으면 거리를 재지 않고 그냥 리턴한다.
...
// turn on the LED if the distance is between dist_min and dist_max
  if(dist_raw < dist_min || dist_raw > dist_max) {
    ...
    int fadeValue = (dist_raw > 200) ? (int)((dist_raw - 200) / 100.0 * 255) :
                              (int)((200 - dist_raw) / 100.0 * 255);                  
...
float USS_measure(int TRIG, int ECHO)
{
  ...
  if (reading > dist_max) reading = 300.0; // return when out of range.
  return reading;
