...
void loop() {
  ...
  float dist_cali = 150 + 200.0 / 238 * (raw_dist - 150);
  
  if(dist_cali < 80 || dist_cali > 450) dist_cali = 0.0; // return 0 when out of range.
...
//  Serial.print(millis());
//  Serial.print(last_sampling_time + INTERVAL);
...
// toggle duty_target between _DUTY_MIN and _DUTY_MAX.
  if(toggle_interval_cnt >= toggle_interval) {
    ...
    if (dist_ema < 200.0) duty_target = 1210;
    else if (dist_ema < 250.0) duty_target = 1490;
    else duty_target = 1752;
  }
  else {
    toggle_interval_cnt++;
  }
